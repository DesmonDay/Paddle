  ...
  for (int i = 0; i < edge_type_len; i++) {
    for (int j = 0; j < gpu_num; j++) {
      ...
      CUDA_CHECK(cudaMemcpyAsync(
          ...
          node.val_storage + sizeof(int64_t) * shard_len[j] + shard_len[j] * i * sample_size,
          sizeof(uint64_t) * shard_len[j] * sample_size,
          cudaMemcpyDefault,
          node.out_stream));
      CUDA_CHECK(cudaMemcpyAsync(
          reinterpret_cast<char*>(actual_sample_size + i * len + h_left[j]),
          node.val_storage + sizeof(int64_t) * shard_len[j] +
              edge_type_len * sizeof(uint64_t) * shard_len[j] * sample_size + (shard_len[j] + shard_len[j] % 2) * i,
...
__global__ void fill_actual_neighbors(uint64_t* vals,
...
NeighborSampleResultV2 GpuPsGraphTable::graph_neighbor_sample_all_edge_type(
    ...
    int edge_type_len,
    uint64_t* key,
    std::vector<int64_t>& edges_split,
    int sample_size,
    int len) {
  ...
  auto d_shard_vals =
      memory::Alloc(place, sample_size * len * edge_type_len * sizeof(uint64_t));
  uint64_t* d_shard_vals_ptr = reinterpret_cast<uint64_t*>(d_shard_vals->ptr());
  auto d_shard_actual_sample_size = memory::Alloc(place, len * edge_type_len * sizeof(int));
  ...
  for (int i = 0; i < total_gpu; ++i) {
    ...
    create_storage(gpu_id,
                   ...
                   (shard_len * sample_size * sizeof(uint64_t) +
                    sizeof(int) * (shard_len + shard_len % 2)) * edge_type_len);
  }
  walk_to_dest(
  ...
  for (int i = 0; i < total_gpu; ++i) {
    ...
    CUDA_CHECK(cudaMemsetAsync(   // should move to for or not?
    ...
    for (int etype_idx = 0; etype_idx < edge_type_len; etype_idx++) {
      int table_offset = get_table_offset(i, GraphTableType::EDGE_TABLE, etype_idx);
      int offset = i * graph_table_num_ + etype_idx;
      ...
      // Unified storage of sample_array and actual_size_array corresponding to 
      // different edge types.
      uint64_t* sample_array = (uint64_t*)(node_info_list + shard_len) + start1;
      int* actual_size_array = (int*)(sample_array + edge_type_len * shard_len * sample_size) + start2;
      ...
      start1 += shard_len * sample_size;
      start2 += (shard_len + shard_len % 2);
    }
  }
  ...
  move_result_to_source_gpu_all_edge_type(gpu_id,
                                          ...
                                          len);
  for (int i = 0; i < edge_type_len; ++i) {
    fill_dvalues<<<grid_size, 512, 0, stream>>>(
        d_shard_vals_ptr + i * len * sample_size,
        val + i * len * sample_size,
        d_shard_actual_sample_size_ptr + i * len,
        actual_sample_size + i * len,
        d_idx_ptr,
        sample_size,
        len);
  }
  ...
  for (int i = 0; i < edge_type_len; ++i) {
    ...
    if (i == 0) {
      edges_split.emplace_back(tss);
    } else {
      edges_split.emplace_back(tss + edges_split[i - 1]);
    }
  }
  ...
  thrust::exclusive_scan(thrust::device_pointer_cast(actual_sample_size),
                         ...
                         0);
  int grid = (len * edge_type_len - 1) / 512 + 1; 
  fill_actual_neighbors<<<grid, 512, 0, stream>>>(
      val,
      result.actual_val,
      actual_sample_size,
